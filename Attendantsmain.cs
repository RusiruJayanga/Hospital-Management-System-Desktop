...
namespace Hospital
{
    public partial class Attendantsmain : Form
    {
        ...
        private void display_Click_1(object sender, EventArgs e)
        {
            ...
            {
                connect.Open();
                SqlCommand cmd = new SqlCommand("select * from attendant", connect);
        ...
        private void serach_Click_1(object sender, EventArgs e)
        {
            try
            {
                connect.Open();

                using (SqlCommand cmd = new SqlCommand("SELECT * FROM attendant WHERE id = @employeeID", connect))
                {
